...
MPOは方策勾配法ではなく**Control as Inference**、すなわち確率推論のフレームワークで制御ポリシーを訓練する手法です。MPOはオフポリシーであるため**サンプル効率が良好**であり、また信頼領域法を用いるため**ロバストな更新**を実現しています。

## 理論

### 環境設定

割引強化報酬（RL）問題の最適な方策（ポリシー） $\pi$ を見つける問題を、**マルコフ決定過程**（MDP）によってモデル化します。MDPは、（連続）状態 $s\in\mathcal{S}$ 、アクション $a\in\mathcal{A}$ 、初期状態分布 $(s_0\sim)\space p_{s_0}$ 、遷移確率関数 $p_T(s_{t+1}|s_t,a_t)$ 、報酬関数 $r(s,a)\in\mathbb{R}$ 、および割引率 $\gamma\in[0,1)$ から構成されます。また、ポリシー $\pi(a|s,\pmb\theta)$ （パラメタは $\pmb\theta$ ）は、任意の状態に対してアクション選択のための確率分布を指定すると仮定し、遷移確率とともに定常分布 $\mu_\pi(s)$ を作ります。

> 遷移確率 $p_T(s_{t+1}|s_t,a_t)$ は、行動 $a_t$ による状態 $s_t$ から $s_{t+1}$ への遷移確率を指定します。

> 定常分布：任意の状態 $s$ から開始した場合の、無限時間経過後の状態分布[5]

これらのパラメタを用いて、ポリシー $\pi$ に従った際の軌跡（トラジェクトリ） $\tau_\pi=\{(s_0,a_0)...(s_T,a_T)\}$ が定義されます。すなわち、トラジェクトリは確率分布 $p_\pi(\tau)=p_{s_0}\prod_{t>0}p_T(s_{t+1}|s_t,a_t)\pi(a_t|s_t)$ を用いて、 $\tau_\pi\sim p_\pi(\tau)$ と定義されます。期待収益は $\mathbb E_{\tau_\pi}\left[\sum_{t=0}^\infty\gamma^tr(s_t,a_t)\right]$ となります。ここで、以下では $R_t=r(s_t,a_t)$ として扱います。
...
このフレームワークにいて最も重要なコンセプトは**最適性確率変数** $e$ です。これは直感的に（行動を選択することにより）最大の報酬を獲得できるイベント、もしくは強化学習タスクを成功させるイベントとして解釈可能です[1]。

この手法では行動の最適性を確率分布で表現します。時間ステップ長 $T$ のトラジェクトリ $\tau$ が与えられたとき、それが最適トラジェクトリである確率は $\mathrm{Pr}(e_{1:T}=1|\tau)$、そうでない確率は $\mathrm{Pr}(e_{1:T}=0|\tau)$ と表現されます。同様に、状態 $s_t$においてアクション $a_t$ が最適行動である確率は $\mathrm{Pr}(e_t=1|s_t, a_t)$ となります。
...
最適性確率変数を $e$ とします。MPOの目的は方策 $\pi$で行動決定を実行した際に、それが最適制御である確率 $p_\pi(e_{1:T}=1)$ を最大化することです（無限時間長の場合 $T\to\infty$）。最適制御である確率とは、具体的には<sup>[補足1](#補足なぜlog)</sup>

$$\log{p_\pi(e_{1:T}=1)}=\log{\int {p_\pi(\tau)\mathrm{Pr}(e_{1:T}=1|\tau)d\tau}}$$
