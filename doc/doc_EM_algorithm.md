...
- [EMアルゴリズム](#emアルゴリズム)
  ...
  - [データの欠測](#データの欠測)
    - [観測値と欠測値](#観測値と欠測値)
    - [欠測データの発生メカニズム](#欠測データの発生メカニズム)
      - [具体例：欠測データの発生メカニズム](#具体例欠測データの発生メカニズム)
    - [欠測データの無視可能性](#欠測データの無視可能性)
  - [EMアルゴリズム](#emアルゴリズム-1)
...
本来であれば値が計測されているはずの変数について、その値が得られず不明である場合にこれを**欠測データ**（*missing data*）といいます。これについて、その定義と分類、および対処方法を以下に記述します。
...
データの欠測がどのようなメカニズムで生じているかはそのデータの本質に関わります。例えば欠測のあるデータから正しい統計的推測が可能であるかなど、欠測データが解析法に与える影響を知るためにはその発生メカニズムを知る必要があります。これを**欠測データの発生メカニズム**（*missing data mechanism*）とよびます。本議論においては、欠測データの確率分布を考える際に $\pmb{y}$ のみからの $\pmb{\theta}$ の推測でバイアスが生じることを防ぐことが目的となります。
...
以下に血圧測定を例にこの３種類を説明します。生活習慣と健康状態の関係を調べている研究者が、自身の研究の中で１月と２月の血圧を1000人の被験者に対して測定したとします（研究者の関心は血圧の相関や回帰モデルあたり）。この場合、完全データと欠測データは例えば次のようにになります。

- １月に1000人に対して調査を行い、２月にその全員が再調査に来た場合の結果が下図①です（**完全データ**）。
- 予算の都合で２月は研究者がランダムに選んだ500人のみ再調査を行った場合の結果は下図②となります（**MCAR**）。この場合、欠測は血圧に関係のない要因により生じており、全体が均一にまばらとなっていることがわかります（**完全にランダムな欠測**）。
- １月に血圧が125以上だった被験者のみ２月に再調査に応じた場合の結果が下図③です（**MAR**）。このように閾値により２月の再調査に応じるかが決まる場合だけでなく、１月の血圧により２月に再測定に来る確率が決まる場合も MAR といえます。このとき、欠測は「観測されている１月の値で２月の欠測確率が決まる」ような**ランダムな欠測**といえます。
- ２月の時点で血圧が125以上であり、何らかの自覚症状がある人のみが再調査に来た場合の結果が下図④です（**NMAR**）。「２月の血圧という欠測しうる変数により欠測確率が決定する」ような**ランダムでない欠測**といえます。
...
詳細は省きますが、E-stepでは観測された値を条件づけたもとで欠測値の期待値を計算し、これを欠測値の代わりとして計算を行っています。また、M-stepでは（E-stepで欠測値に期待値を代入したことから）欠測値を含まない形の尤度関数が得られているので、これをもとに通常の完全データの最尤推定と同様にパラメタの最大化を行っています[5]。最後に、EMアルゴリズムの概念図を下記に示します。
...
[3] 機械学習のための確率と統計, 杉山将, 講談社, 第１刷（Chapter 9）

[4] 強化学習アルゴリズム入門 -「平均」からはじめる基礎と応用-, 曽我部東馬, オーム社, 第２刷 

[5] 欠測データの統計科学 - 医学と社会科学への応用, 高井啓二, 星野祟宏, 野間久史, 岩波書店, 第２刷